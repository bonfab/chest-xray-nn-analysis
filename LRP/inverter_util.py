...
class RelevancePropagator:
    ...
    def __init__(self, lrp_exponent, beta, method, epsilon, device, res_rule=0):
    ...
    def compute_propagated_relevance(self, layer, relevance):
        ...
        if isinstance(layer,
            ...
            return self.resblock_inverse(layer, relevance).detach()
        else:
    ...
    def get_layer_fwd_hook(self, layer):
        ...
        if isinstance(layer, ResBlock):
            ...
            #print(list(layer.named_children()))
            save = RelevancePropagator.SaveRelevance()
            setattr(layer, "saved_relevance", save)
            for name, module in layer.named_children():
                if name == "conv_in":
                    setattr(module, "saved_relevance", save)
            return self.resblock_fwd_hook
    ...
    @module_tracker
    def resblock_fwd_hook(self, m, in_tensor: torch.Tensor,
    ...
    def resblock_inverse(self, m, relevance_in):
        relevance_in = relevance_in.view(m.out_shape)
        m.saved_relevance.set_relevance(relevance_in)
        return relevance_in
    ...
    def conv_nd_inverse(self, m, relevance_in):
        ...
        res = False
        try:
            saved = m.saved_relevance.relevance
        ...
        if self.method == "e-rule":
            with torch.no_grad():
                ...
                relevance_out = inv_conv_nd(relevance_in/norm,
                                            ...
                                            groups=m.groups)
                #print(relevance_out.shape, m.in_tensor.shape)
                relevance_out *= m.in_tensor
        ...
        if not res:
            ...
            if self.res_rule == 0:
                return 0.5 * (relevance_out + saved)
            if self.res_rule == 1:
                return 0.5 * (relevance_out + torch.pow(saved/(self.eps +saved), 2))
